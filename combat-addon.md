# Lonelog: Combat Add-on
**Optional Notation for Tactical Encounters**
Version 1.1.0
CC BY SA
...
* 1. Combat Blocks | 4
* 2. Round Tracking | 6
* 3. Tracking Combatants | 8
* 4. Actor Actions | 11
* 5. Complex Encounters | 14
* 6. Combat Aftermath | 16
* 7. Putting It All Together | 17
* Cross-Add-on Interactions | 22
* System Adaptations | 24
* Best Practices | 26
* Quick Reference | 29
* FAQ | 32
* Credits & License | 34
...
| :--- | :--- | :--- |
| `[COMBAT] / [/COMBAT]` | Delimit a tactical encounter within a scene | `[COMBAT]`<br>`...`<br>`[/COMBAT]` |
...
> `[F:Piratex2|HP 4 each | Medium|armed|crossbow]`
...
> `Rd3 Roster: [PC:Alex | HP 3] [N: Jordan | HP 4] [F:Captain | HP 4] [F:Piratex1|HP 4]`

This gives you a snapshot at each round without hunting back through the log.

Scaling tips:
* Group identical foes. `[F: Piratex2]` beats tracking Pirate A and Pirate B separately-unless they're in different positions or have different status.
...
> `[PC:HP 12] [F: Captain | HP 10] [F: Piratex2|HP 4] [F:Rat | HP 2]`
> `Rd1 @Rat d: 17 >= 10 Hit => dead @(Cap) d: 19 >= 14 Hit HP-7 @(Pirx2) d: 15,9 vs11 1Hit [N: Jordan HP-4]`
...
> `d: d20+3=16 vs AC 11 -> Hit => [F:Piratex1| dead]`
> `@(Pirate) Drops crossbow, flees => [F:Piratex1|fled]`
> `Rd3`
> `@Chase the Captain`
...
> `The crates provide cover, but not for long. A rat-faced man with a knife lunges from behind a barrel. Behind him, the Pirate Captain draws his cutlass with a grin. Two crossbowmen take position on the cargo stack.`
> `[COMBAT]`
> `[PC:Alex | HP 12 | Engaged] [N: Jordan|ally|HP 8|Close]`
> `[F:Pirate Captain | HP 10|Close|cutlass]`
> `[F:Piratex2|HP 4 each | Medium|crossbow]`
> `[F:Dock Rat | HP 2 | Engaged | knife]`
> `Rd1`
...
> `@(Piratex2) Fire at Jordan from the cargo stack`
...
> `@Power attack on Captain`
> `d: d20+4=20 vs AC 13 -> Critical Hit, 1d8 x 2 = 8`
...
> `=> One pirate tumbles off the cargo stack. [F:Piratex1|dead]`
> `The surviving pirate drops his crossbow and bolts into the fog.`
> `[F:Piratex1|fled]`
> `Rd3`
> `@Chase the Captain before he escapes`
> `d: Athletics d6=3 vs TN 5 -> Fail`
> `=> He's too fast-vaults the dock railing and drops into a hidden rowboat. The fog swallows him in seconds.`
> `[/COMBAT]`
> `I press a hand to my ribs, breathing hard. Jordan limps over, shoulder dark with blood. The dock is quiet except for lapping water and the creak of ropes.`
...
X (Five rounds with no intermediate updates)
...
| Purpose | Tag | Example |
| :--- | :--- | :--- |
| Track individual combatant state | `[F: Name\|stats]` | `[F:Thug A\|HP 6\|Close\|armed]` |
| Track a group of identical combatants | `[F:Namex#\|stats]` | `[F: Goblin 3\|HP 3 each\|Close]` |
| Inline damage shorthand | `[F:Name\|HP-#]` | `[F:Thug A\|HP-3]` |
| Mark a combatant as eliminated | `[F:Name\|dead]` | `[F:Thug A\|dead]` |
| Mark a combatant as escaped | `[F:Name\|fled]` | `[F:Captain\|fled]` |
...
| Initiative note | `(Init: Name #, ...)` | `Rd1 (Init: Captain 18, Alex 15)` |

### Complete Example
> `S9 *Dockside ambush* [COMBAT]`
> `[PC:Alex | HP 12] [N:Jordan|ally|HP 8]`
> `[F:Pirate Captain | HP 10 | Close] [F: Piratex2|HP 4|Medium]`
> `Rd1`
> `@Slash Captain d: 19 >= 13 Hit 7dmg => [F:Captain | HP 3| staggered]`
> `@(Captain) Desperate swing d: 11 >= 14 Miss`
> `@(Jordan) Fires at Pirate d: 16 >= 11 Hit => [F:Piratex1| dead]`
> `@(Pirate) Drops crossbow, flees => [F:Piratex1|fled]`
> `[/COMBAT]`
...
* **Q: How do I handle area effects or multi-target attacks?** 
  ...
  > `@Fireball targeting Goblinx3`
  > `d: 8d6=28, DC 14 Dex save`
  > `d: Goblin 1: 12 <= 14 Fail, Goblin 2: 17 >= 14 Success, Goblin 3: 8 <= 14 Fail`
  > `=> Two goblins incinerated. One dodged, half damage. [F:Goblinx1|HP 2|singed]`
