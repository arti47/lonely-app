 ...
 *
 * Asks whether the app *works*: every screen renders, every flow completes,
 * zero console errors, no horizontal overflow at 360px and 390px, and the
 * notation engine folds a real log in the page.
 *
 * `reachability.mjs` asks the other question — whether a stranger can find any
 * of it. Both drive the same headless harness in `browser.mjs`.
 */
...
const app = await launch({
  hash: '#/campaigns',
  serve: {
    rewrite: (pathname, body) => (deployedBuild && pathname.endsWith('/service-worker.js')
