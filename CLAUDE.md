...
| `tests/` + `package.json` | Dev-only regression harness (`npm test`): `*.test.js` unit tests, `corpus/` spec examples, `browser.mjs` shared headless harness, `smoke.mjs` (does it work?), `reachability.mjs` (can a stranger find it? — `npm run audit`), `typecheck.mjs`, `extract-corpus.mjs`. The only devDependency is `typescript`, and it is optional. `node_modules` gitignored; not in the SW app shell |
...
| `state.js` | ✅ Sheet pane: folded PC sheet, NPC/location index, threads, clocks; persistent status strip; shared tag-line builders and `currentFlag` reused by the add-on surfaces |
...
| `guide.js` | ✅ How-to guide: one accordion per part of the app plus the solo play loop; shares the Help tab with the reference |
...
- [x] **Phase 8 — Lint & reference.** `lint.js` rules from `docs/spec-review.md`
      ...
      *Done: 29 paraphrased entries, each cited and each example asserted to lex.
...
5. **Committed regression harness.** `npm test` asserts at minimum: boot/wiring
   ...
   return. **Reachability is part of it** (§11): `npm run audit` walks the real
   app and prints *every* finding in one run, so the loop is run → fix → run
   again until it prints nothing. It is inside `npm test`, which keeps it there.
...
- Document findings as a numbered work-list (**Rule / Target / Fix / Why**),
  ...
  future audits don't re-litigate it. **Four passes complete —
  ...
  control in the app, D1–D4 on the play loop itself, and C16/D5 closing what
  those passes left behind, and E1–E4 on reachability — whether someone who has
...
| 2026-07-27 | Fixed: deleting a campaign left the deleted campaign on screen until you visited another tab and came back. Root cause: assigning `location.hash` its *current* value fires no `hashchange`, so a screen that acted on its own data and then sent you back to itself never re-rendered — `go()` was navigation-only. `go()` now re-renders when the target is where you already are, and the nav handler skips that case so tapping the tab you are on cannot discard a half-typed line | Browser checks: deleting a campaign drops its card without the hash changing, and re-tapping the current tab keeps the composer's text. The delete check was run against the old `go()` first and fails there | `lonely-v19` |
...
- Every change updates this file in the same commit (§9.1) and lands with the
  suite green (§9.4–9.5).
- The remote carries `main` and nothing else. Two stale `claude/…` refs were
  removed by hand from the GitHub branches page after the proxy refused
  `git push --delete`; keep it that way.
